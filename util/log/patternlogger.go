...
type PatternLogger struct {
	...
	CategoryNames   LogCategoryNames
...
func getCallerInfoBean(stacktrace handler.StackTrace) (callerInfoBean CallerInfoBean) {
...
func (p *PatternLogger) logMonitoring(messageType string, correlationID string, monitorType LogMonitorType, targetURL string,
	...
	messageBean.CategoryNames = p.CategoryNames
...
func (p *PatternLogger) LogRequestApplication(correlationID string) string {
	callerInfoBean := getCallerInfoBean(handler.GetStackTrace())
...
func (p *PatternLogger) WriteRequestMsg(correlationID string, url string, message interface{}) {
	if message == nil {
		...
		if err != nil {
			p.Error(correlationID, err.Error(), handler.Wrap(err))
		}
...
func (p *PatternLogger) WriteResponseMsg(correlationID string, message interface{}) {
	if message == nil {
		p.logApp(correlationID, INFO, "Response Message: null", nil)
	} else {
		msg, err := json.Marshal(message)
		if err != nil {
			p.Error(correlationID, err.Error(), handler.Wrap(err))
		}
		p.logApp(correlationID, INFO, "Response Message: "+string(msg), nil)
	}
}
...
func checkArguments(args []interface{}) (message string, staceTrace string) {
	for i := 0; i < len(args); i++ {
		...
		case string:
			message = message + " " + v
		case *strconv.NumError, handler.Error:
...
func logStringPattern(bean *LogMessageBean) (log string) {
	log = fmt.Sprintf(`timestamp=%s|logType=%s|correlationID=%s|level=%v|message=%s|messageType=%s|stackTrace=%v|CategoryNames=%s|monitorType=%v|sourceSystem=%s|sourceHostName=%s|targetSystem=%s|targetURL=%s|action=%s|elapsedTime=%d|responseCode=%s`,
		bean.Timestamp, bean.LogType, bean.CorrelationID, bean.Level, bean.Message, bean.MessageType, bean.StackTrace, bean.CategoryNames,
